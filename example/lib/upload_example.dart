...
class _UploadExampleScreenState extends State<UploadExampleScreen> {
    ...
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        for (final platformFile in result.files) {
          final uploadFile = GSDUploadFile.fromPlatformFile(platformFile);
  ...
  void _addDemoFile() {
    ...
    if (kIsWeb) {
      // Für Web: Erstelle PlatformFile und verwende fromPlatformFile
      final platformFile = PlatformFile(
        name: fileName,
        size: demoBytes.length,
        bytes: demoBytes,
      );
      uploadFile = GSDUploadFile.fromPlatformFile(platformFile);
    } else {
