...
class _UploadExampleScreenState extends State<UploadExampleScreen> {
  ...
  Future<void> _checkDOCUframeConnection() async {
    ...
    try {
      // Erstelle temporäre DOCUframe-API für Check
      api = DOCUframeApi(
      ...
      final checkResult = await api.v1.service.checkService();

      if (checkResult.isOk) {
        _addToLog('✅ DOCUframe-Service erreichbar');

        final loginResult =
            await api.v1.authentication.login(_passwordController.text);

        if (loginResult.isOk) {
          _addToLog('✅ Login erfolgreich als ${_usernameController.text}');
          _configureDOCUframe(api);
        } else {
  ...
  void _configureDOCUframe(DOCUframeApi api) {
    try {
      _api = api;
    ...
    try {
      ...
      for (final platformFile in result) {
        ...
        if (!kIsWeb) {
          ...
          uploadFile = GSDUploadFile.fromBytes(
              await platformFile.readAsBytes(), platformFile.name);
        }
      ...
      _addToLog('${result.length} Datei(en) zur Upload-Queue hinzugefügt');
    } catch (e) {
