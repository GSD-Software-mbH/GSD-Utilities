...
class _GSDWebConfigManager extends GSDConfigManager {
  ...
  /// Schlüssel für die sensiblen Daten im LocalStorage
  ...
  _GSDWebConfigManager(String key, {super.backgroundMode}) : super._() {
  ...
  Future<GSDConfigResult<T>> loadConfig<T extends GSDBaseConfig>(
    ...
    try {
      ...
      if (configFileContent.isNotEmpty) {
        ...
        // Load secure data (passwords, tokens)
        String secureFileContent = _localStorageManager.readData(_secureKey);
        if (secureFileContent.isNotEmpty) {
          String decryptedSecure =
              await _encryptionManager.decryptAES(secureFileContent);
          config.loadSecureJson(decryptedSecure);
          log += "Secure data loaded and decrypted\n";
        }
  ...
  Future<void> _savePlatformConfig(GSDBaseConfig config) async {
    ...
    if (secureJson != "{}") {
      String encryptedSecure = await _encryptionManager.encryptAES(secureJson);
      _localStorageManager.writeData(_secureKey, encryptedSecure);
      onSecureConfigSaved?.call();
    }
  }
}
